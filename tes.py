...
Builder.load_file(dir_path + '/tes.kv')
...
class Gamemain(Widget):
    ...
    minus = NumericProperty(0)

    clickbutt = ObjectProperty(None)
    
    def tellmetheway(self):
        if self.clickbutt.tell == True :
            self.image.source = "upbutt.png"


        if self.clickbutt.tell == False :
            self.image.source = "downbutt.png"
...
class Itemgame(Widget):
    ...
    picturelist = ["news","can","bottle","plasticbag","phone","brokenbottle"]
    ...
    def setpic(self):
        ...
        if self.picture == "news":
            self.image.source = "newsgood.png"
            self.itemtype = 0
        if self.picture == "can":
            self.image.source = "cangood.png"
            self.itemtype = 0
        if self.picture == "bottle" :
            self.image.source = "bottlegood.png"
            self.itemtype = 0
        if self.picture == "plasticbag" :
            self.image.source = "plasticbagbad.png"
            self.itemtype = 1
        if self.picture == "phone" :
            self.image.source = "phonebad.png"
            self.itemtype = 1
        if self.picture == "brokenbottle" :
            self.image.source = "glassbottlebad.png"
            self.itemtype = 1
...
class Trashgame(App):
    def build(self):
        game = Gamemain()
        Clock.schedule_interval(game.update, 1 / 40 ) #คำสั่งupdate
        return game
